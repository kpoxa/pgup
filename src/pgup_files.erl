...
list_files_result({error, enoent}) ->
	pgup_log:err("Can't open directory with sql files or directory is empty"),
...
check_file_content(<<>>) ->
	pgup_log:err("SQL file is empty"),
	throw(file_is_empty);
...
file_meta_from_tokens(T) ->
	pgup_log:err("wrong file name tokens ~p, example : 0001.init_tables.up.sql or 0003.drop_indices.down.sql", [T]),
	skip.
...
op("down") ->
	{op, down};
op(Wrong) ->
	pgup_log:err("Wrong operation in file name: ~p (valid values are \"up\" or \"down\")", [Wrong]),
